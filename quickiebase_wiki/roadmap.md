# Roadmap for Quickiebase development


Quickiebase will be developed in stages. This is the roadmap.

## Phase 0

Phase zero consists of implementing a RAM-based database (in `ramdb.py`). This stores its data in memory, which has the obvious 
drawback that all the data is lost once the process stops running. So phase 0 doesn't produce a usable database, **but** it does
produce a foundation on which we can build.
...
```none
 ...
 ┌─────────────┐    ┌─────────────┐                   ┌─────────────┐
 │ application │--->│ quickiebase │==================>│ quickiebase │
 │ program     │    │ library     │                   │ server      │
 └─────────────┘    └─────────────┘                   └─────────────┘
